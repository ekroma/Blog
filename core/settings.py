...
EMAIL_HOST = config('EMAIL_HOST') #  какой сервис используется для отправки писем
EMAIL_HOST_PASSWORD = config('EMAIL_HOST_PASSWORD') # пороль от почты
EMAIL_USE_TLS = config('EMAIL_USE_TLS', cast=bool) # вид соеденения для отправки писем 
